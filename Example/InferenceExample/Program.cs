...
class Program
{
    ...
    private static async Task ProcessVideoPositivenessAsync(string videoName, int frameSecInterval, float[,,,] framesArray, int numFrames, int targetWidth = 227, int targetHeight = 227)
    {
        ...
        while (!threadCompletion.All(x => x))
        {
            ...
            Console.WriteLine($"{Math.Round(rev * 100)}%");
        }
    ...
    private static async Task ProcessVideoFilterAsync(string videoName, int frameSecInterval, float[,,,] framesArray, int numFrames, int targetWidth = 224, int targetHeight = 224)
    {
    ...
    public static void ClassifyVideo(string videoName)
    {
        ...
        try
        {
            ...
            if (result == 2.0)
            {
                Console.WriteLine($"Video '{videoName}' is classified as unsafe and should be blocked.");
            }
            else
            {
                Console.WriteLine($"Video '{videoName}' is classified with MPAA rating: {GetMpaaRating(result)}.");
            }
        }
        catch (Exception ex)
        {
    ...
    public static async Task Main(string[] args)
    {
        ...
        string[] videoFileExtensions = { "*.mp4", "*.avi", "*.mov", "*.mkv" };

        // Get all video files with the specified extensions
        List<string> videoFiles = new List<string>();
        foreach (var filter in videoFileExtensions)
        {
            videoFiles.AddRange(Directory.GetFiles(inputFolderPath, filter));
        }
        ...
        foreach (string videoFilePath in videoFiles)
        {
            ...
            if (framesArrayPositiveness != null)
            {
                ...
                await ProcessVideoPositivenessAsync(fileNameWithoutExtension, frameSecInterval, framesArrayPositiveness, numFrames);
            }
            ...
            {
                string fileNameWithoutExtension = Path.GetFileNameWithoutExtension(videoFilePath);
                await ProcessVideoFilterAsync(fileNameWithoutExtension, frameSecInterval, framesArrayFilter, numFrames);
                ClassifyVideo(fileNameWithoutExtension);
            }
