...
#include "include/lstm_mpaa_model.h"
...
double inference(MLPModel* model, double* features, double* mean, double* scale) {
    double* transformed_features = transform_data(features, mean, scale, NUM_FEATURES);
...
DLLEXPORT double predictSAMP(const char* epp_path, const char* efp_path) {    
    ...
    extract_features(epp_data, efp_data, num_frames, frame_sec_interval, features);
    ...
    inference_intermediate(&model1, features, std_mean_mlp1, std_scale_mlp1, intermediate_output1);
    inference_intermediate(&model2, features, std_mean_mlp2, std_scale_mlp2, intermediate_output2);
    inference_intermediate(&model3, features, std_mean_mlp3, std_scale_mlp3, intermediate_output3);
    ...
    double mpaa_rating = predictMPAA(tensor_combined_input, tensor_mlp_output);
    return mpaa_rating; // Return the MPAA rating
}
